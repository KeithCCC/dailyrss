    ...
    if os.path.exists(filename):
        try:
            ...
            <body>
                ...
                <a href="/rss" class="button">View All Feeds</a>
                <a href="/select_feed" class="button">Select Single Feed</a>
                <a href="/select_multiple_feeds" class="button">Select Multiple Feeds</a>
                <a href="/manage_feeds" class="button">Manage Feeds</a>
                <a href="/add_feed" class="button">Add New Feed</a>
                <a href="/end_program" class="button end-button">End Program</a>
            </body>
            </html>
...
def index():
    ...
    if os.path.exists(filename):
        try:
            ...
            # Create HTML table with links to individual RSS feeds
            table_rows = []
            for index, row in df.iterrows():
                table_rows.append(f'''
                <tr>
                    ...
                    <td>{row['title']}</td>
                    <td>{row['URL']}</td>
                    <td><a href="/rss_single/{index}">View RSS</a></td>
                    </tr>
                ''')
            ...
            <body>
                <a href="/rss" class="button">View All RSS Feeds</a>
                <a href="/select_feed" class="button">Select Single RSS Feed</a>
                <a href="/select_multiple_feeds" class="button">Select Multiple RSS Feeds</a>
                <a href="/select_labels" class="button">Select by Labels</a>
                <a href="/add_feed" class="button">Add New Feed</a>
                <a href="/manage_feeds" class="button">Manage Feeds</a>
                <br>
                <table class="table">
            ...
            return f"Error decoding JSON: {str(e)}. Please check the contents of default.json."
        except Exception as e:
            return f"Error processing data: {str(e)}"
    else:
        return '''
...
def save_feed():
    ...
    new_row = pd.DataFrame([[label, title, url]], columns=['label', 'title', 'URL'])
    df = pd.concat([df, new_row], ignore_index=True)
    df.to_json(filename, orient='records')
    ...
    <body>
        <div class="message">Feed added successfully!</div>
        <a href="/" class="button">Home</a>
    </body>
    </html>
    '''
    ...
    if os.path.exists(filename):
        try:
            ...
            <body>
                ...
                <a href="/manage_feeds" class="button">Back to Manage Feeds</a>
            </body>
            </html>
            '''
            return html_template.format(row_index=row_index, label=row['label'], title=row['title'], url=row['URL'])
        except Exception as e:
            return f"Error processing data: {str(e)}"
    else:
        return 'default.json does not exist'
...
def update_feed(row_index):
    ...
    if os.path.exists(filename):
        try:
            ...
            return f"Error updating feed: {str(e)}"
    else:
        return 'default.json does not exist'
...
def delete_feed(row_index):
    ...
    if os.path.exists(filename):
        try:
            ...
            return f"Error deleting feed: {str(e)}"
    else:
        return 'default.json does not exist'
